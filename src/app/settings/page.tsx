...
export default function SettingsPage() {
  ...
  return (
    <div className="flex flex-col gap-6 w-full animate-fade-in pb-36">
      {/* Page Header - Left-aligned to match other pages */}
      ...
      {saveSuccess && (
        <div className="flex items-center gap-3 p-4 bg-emerald-500/15 border border-emerald-500/30 text-success font-semibold rounded-xl text-sm transition-all duration-300 w-full">
      ...
      <form onSubmit={handleSave} className="w-full">
        <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-8 items-start">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {profile.logoUri ? (
                    <div className="relative h-28 border border-border-main rounded-xl flex items-center justify-center bg-white/[0.02] overflow-hidden">
                      ...
                      <button type="button" onClick={() => removeImage('logoUri')} className="absolute top-2 right-2 bg-danger hover:bg-red-600 text-white text-xs font-semibold px-2.5 py-1 rounded flex items-center gap-1 transition-colors duration-200 cursor-pointer" title="Remove Logo">
                        <Trash2 size={12} />
                        Remove
                      </button>
                    </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {profile.signatureUri ? (
                    <div className="relative h-28 border border-border-main rounded-xl flex items-center justify-center bg-white/[0.02] overflow-hidden">
                      ...
                      />
                      <button type="button" onClick={() => removeImage('signatureUri')} className="absolute top-2 right-2 bg-danger hover:bg-red-600 text-white text-xs font-semibold px-2.5 py-1 rounded flex items-center gap-1 transition-colors duration-200 cursor-pointer" title="Remove Signature">
                        <Trash2 size={12} />
                        Remove
                      </button>
                    </div>
          ...
          {/* Right Column: Document Defaults */}
          <div className="flex flex-col gap-6">
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-[calc(100%-3rem)] max-w-6xl flex justify-end p-4 bg-card-bg/60 backdrop-blur-xl border border-border-main rounded-xl shadow-lg">
          <button
            type="submit"
            className="flex items-center gap-2 bg-primary hover:bg-primary-hover text-text-inverse px-7 py-3 rounded-lg font-semibold text-sm border border-primary/30 shadow-lg shadow-primary/20 transition-all duration-200 hover:-translate-y-0.5 animate-glow-pulse cursor-pointer"
          >
